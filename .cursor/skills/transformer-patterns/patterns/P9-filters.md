...
118. Allowed control families (compose with a High Fidelity Design System / your component library): search text (names + IDs), **select** for fixed sets, date / range pickers, optional **autocomplete / typeahead** for open-ended entities, **multiselect** where the spec requires multiple values from a set, binary **switch** for flags such as "Active only".
