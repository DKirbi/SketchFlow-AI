...
import type { NotificationFilters, NotificationRow } from '../types';
...
export function passesFilters(row: NotificationRow, filters: NotificationFilters): boolean {
  ...
  if (filters.datasetScope === 'betting' && row.dataset !== 'BETTING') return false;
  if (filters.datasetScope === 'media' && row.dataset !== 'MEDIA_VALIDATED') return false;
