...
| **LOFIChip** | `chip` | P9 chip. `onClear` (or deprecated `onDismiss`) shows ✕ for an active-filter token. Omit `onClear` and pass `selected` + `onClick` for exclusive status filters (count in the label). Do not fake chips with `LOFIButton`. |
